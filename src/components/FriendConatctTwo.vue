...
//   props: ['name', 'phoneNumber', 'email', 'isFavroute'],
  // 1
 // as we discuss before there are multiple ways to define apros in your component
//  so this way we define a prop as array is not good when we work in big projects and also with teams because we don't know types we want to use in props and it make it dificult to use it to other so we can define new way to define props 
    ...
    props: {
        ...
        // default also can be funcatin which will return somthing when we want to calculate defaylt by some logic default() and when we use default() in proptype function then whole default function will be set as default for that prop
